...
function renderPeriodStockValues(){
    const from=$("periodValueFrom"),to=$("periodValueTo");
    if(!from||!to)return;
    ...
    const calculate=()=>{
        const start=parseLocalDateStart(from.value),end=parseLocalDateEnd(to.value);
        if(!start||!end)return;

        if(start>end){
            $("periodReceivingValue").textContent="Invalid Date Range";
            $("periodIssueValue").textContent="-";
            $("periodBalanceValue").textContent="-";
            $("periodValueRangeText").textContent="From Date must be before or equal to To Date.";
            return;
        }

        /*
         * FIRST-MONTH / FULL LEDGER VALUATION
         * Total Added Stock Value = every stockIn batch/invoice in the system,
         * including the opening batch created when an item is first added.
         * Total Issue IN Value = every FIFO issue cost in the system.
        ...
        const issueValue=allStockOut.reduce(
            (t,r)=>t+issueFifoCost(r),
            0
        );

        const balance=addedValue-issueValue;

        $("periodReceivingValue").textContent=money(addedValue);
        $("periodIssueValue").textContent=money(issueValue);
        $("periodBalanceValue").textContent=money(balance);
        $("periodBalanceValue").classList.toggle("negative-period-balance",balance<0);

        const f=v=>{const [y,m,d]=v.split("-");return `${d}/${m}/${y}`;};
        $("periodValueRangeText").innerHTML=
            `All recorded stock additions & issues · Selected view: <b>${f(from.value)}</b> to <b>${f(to.value)}</b>`;
    };

    from.onchange=calculate;
    to.onchange=calculate;

    const btn=$("periodValueThisMonth");
    if(btn)btn.onclick=()=>{
        const today=new Date();
        from.value=dateInputValue(new Date(today.getFullYear(),today.getMonth(),1));
        to.value=dateInputValue(today);
